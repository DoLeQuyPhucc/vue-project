...
<template>
  <div v-if="movie && visible" class="movie-card-modal">
    <div
      class="movie-card-modal-content bg-zinc-900 text-white rounded-lg shadow-xl overflow-hidden w-72 border border-zinc-700"
    >
      ...
      <div class="relative h-40 bg-gradient-to-t from-zinc-900 to-zinc-800">
        ...
        />
        <div
          class="absolute top-0 left-0 w-full h-full bg-gradient-to-t from-zinc-900 via-zinc-900/70 to-transparent"
        ></div>

        <!-- Rating badge -->
        <div
          class="absolute top-3 right-3 bg-yellow-500 text-black text-xs font-bold px-2 py-1 rounded-md flex items-center"
        >
          <svg
      ...
      <div class="p-4">
        ...
        <p class="text-zinc-400 text-xs mb-2">
          {{ movie.origin_name }} ({{ movie.year || 'N/A' }})
        </p>
