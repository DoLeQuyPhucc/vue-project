...
const movieStatus = computed(() => {
  ...
  switch (movieData.value.status) {
    ...
    default: return movieData.value.status
  }
})
  ...
  <div class="movie-detail-page bg-zinc-900 text-white min-h-screen">
    ...
    <div v-else-if="error" class="container mx-auto px-4 py-20 text-center">
      <div class="bg-zinc-800 rounded-lg p-8 max-w-2xl mx-auto">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 text-red-500 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <h2 class="text-xl font-bold mb-2">Không thể tải phim</h2>
        <p class="text-gray-400 mb-4">{{ error }}</p>
        <router-link to="/" class="inline-block bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-md transition-colors">
          Quay lại trang chủ
        </router-link>
      </div>
    </div>
    ...
    <div v-else-if="movieData" class="pb-12">
      ...
      <div class="movie-backdrop relative">
        ...
        <div class="absolute inset-0 bg-gradient-to-b from-zinc-900/30 via-zinc-900/70 to-zinc-900"></div>
        <div class="absolute inset-0 bg-cover bg-center" 
          :style="`background-image: url('${movieData.poster_url}'); filter: blur(2px) brightness(0.4);`">
        </div>
        ...
        <div class="container mx-auto px-4 relative z-10">
          <div class="flex flex-col md:flex-row py-16 md:py-24 gap-8">
            ...
            <div class="poster-wrapper w-full md:w-80 flex-shrink-0 relative mx-auto md:mx-0" style="max-width: 300px">
              <div class="movie-poster rounded-lg overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.8)] relative group">
                <img :src="movieData.thumb_url" :alt="movieData.name" class="w-full h-auto" />
                ...
                <div class="absolute top-4 right-4 bg-black/80 text-yellow-400 p-2 rounded-md 
                            flex items-center gap-1 shadow-lg border border-yellow-500/30">
                ...
                <div v-if="movieData.type === 'series'" 
                     class="absolute top-4 left-4 bg-gradient-to-r from-red-700 to-red-600 
                           text-white px-3 py-1 rounded shadow-md backdrop-blur-sm text-xs font-medium">
                  Phim Bộ
                </div>
                <div v-else 
                     class="absolute top-4 left-4 bg-gradient-to-r from-blue-700 to-blue-600 
                           text-white px-3 py-1 rounded shadow-md backdrop-blur-sm text-xs font-medium">
                  Phim Lẻ
                </div>
              </div>
              ...
              <button 
                ...
                class="w-full bg-gradient-to-r from-red-700 to-red-500 hover:from-red-600 hover:to-red-400 
                       text-white py-3 px-6 rounded-md mt-4 font-medium flex items-center 
                       justify-center gap-2 transition-all duration-300 shadow-lg shadow-red-700/30 
                       hover:shadow-red-500/40 hover:translate-y-[-2px]"
              >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              ...
              <button 
                ...
                class="w-full bg-zinc-800 hover:bg-zinc-700 text-white py-3 px-6 rounded-md mt-2 
                       font-medium flex items-center justify-center gap-2 transition-all duration-300 
            ...
            <div class="movie-info flex-grow backdrop-blur-sm bg-black/30 p-6 rounded-xl border border-white/10">
              <h1 class="text-3xl md:text-4xl font-bold text-white mb-2">{{ movieData.name }}</h1>
              <h2 class="text-xl text-gray-300 mb-4">{{ movieData.origin_name }} ({{ releaseYear }})</h2>
              
              <!-- Movie meta info with enhanced styling -->
              <div class="flex flex-wrap gap-2 mb-6">
                <span class="bg-zinc-800/80 text-gray-300 px-3 py-1 rounded-full text-sm border border-zinc-700/50">
                  {{ movieData.time }}
                </span>
                <span class="bg-zinc-800/80 text-gray-300 px-3 py-1 rounded-full text-sm border border-zinc-700/50">
                  {{ movieStatus }}
              ...
              <div v-if="movieData.type === 'series'" class="mt-6 mb-2">
                <div class="bg-gradient-to-r from-red-900/70 to-red-800/40 px-6 py-4 rounded-lg inline-flex items-center border border-red-700/50 shadow-lg">
                  <span class="font-semibold mr-2">Tập mới nhất:</span> 
                  <span class="text-red-400 font-bold text-xl">{{ movieData.episode_current }}</span>
        ...
        <div class="tab-content pt-8">
          ...
          <div v-else-if="activeTab === 'trailerPhim'" class="trailer-container">
            <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 backdrop-blur-sm">
              ...
              <div v-if="trailerYoutubeId" class="aspect-video w-full max-w-3xl mx-auto bg-black rounded-lg overflow-hidden shadow-xl">
                <iframe 
                  class="w-full h-full" 
              ...
              <div v-else class="text-center py-10 text-gray-400 bg-zinc-800/40 rounded-lg border border-dashed border-zinc-700">
            ...
            <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 backdrop-blur-sm">
              ...
              <div v-if="episodes.length > 0" class="mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  >
                    <span class="flex items-center gap-2">
                      <svg v-if="selectedServer === index" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div v-if="episodeList.length > 0" class="mb-6">
                ...
                <div class="flex flex-wrap gap-2 p-4 bg-zinc-800/30 rounded-lg border border-zinc-800">
                  <button 
              ...
              <div class="aspect-video w-full bg-zinc-900 rounded-lg flex items-center justify-center border border-zinc-800 shadow-xl overflow-hidden">
                <div class="text-center p-10">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto mb-4 text-red-500 opacity-80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p class="text-gray-400 text-lg">Nhấp vào một tập để bắt đầu xem phim</p>
                  <p class="text-gray-500 text-sm mt-2">Chọn máy chủ phù hợp để có trải nghiệm xem tốt nhất</p>
                </div>
              </div>
            </div>
          </div>
...
/* Film grain effect */
.movie-description, .trailer-container, .watch-container {
  position: relative;
}

.movie-description::before, .trailer-container::before, .watch-container::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
