...
import { getFilmLeList } from '@/services/movieService'
...
const fetchMovies = async () => {
  try {
    ...
    console.log('Fetching phim le page:', currentPage.value)
    const response = await getFilmLeList(currentPage.value)
    console.log('API response:', response)

    if (response && response.data) {
      movies.value = response.data.items || []
      pageTitle.value = response.data.titlePage || 'Phim Lẻ'
      ...
      error.value = 'Không thể tải dữ liệu phim lẻ'
    }
  } catch (err) {
